<template>
  <div class="backColor">
    ...
    >
      ...
      >
        ...
        <el-form-item label="状态" prop="status">
          <el-radio-group v-model="addGroupList.status" @change="changelabel">
    ...
    >
      ...
      <span>
        <el-button @click="adminGroupEditadd = false">取 消</el-button>
  ...
  methods: {
    ...
    addGroupSubmit() {
      this.$refs.addGroupListref.validate((v) => {
        ...
        postD(AuthGroupAddApi(), this.addGroupList).then((res) => {
          if (res.code == "200") {
    ...
    async removeRow(userinfors) {
      ...
      if (removeValue === "confirm") {
        ...
        postD(AuthGroupDelApi(), this.removeRowFrom).then((res) => {
          if (res.code == "200") {
            this.$message.success("状态修改成功");
          } else if (res.code == "-200") {
            this.$message.error("参数错误，或暂无数据");
    ...
    adminGroupEditaddinfo() {
      this.$refs.adminGroupEditFromref.validate((v) => {
        postD(AuthGroupEditApi(), this.adminGroupEditFrom).then((res) => {
          if (res.code == "200") {
            this.$message.success("状态修改成功");
