<template>
  <div>
    <el-button type="success" @click="addProduct">添加类别</el-button>
...
import { product_typeReleaseApi } from "@/urls/product_typeUrl.js";
export default {
  inject: ["productValue"],
  data() {
  ...
  methods: {
    ...
    addProductValue() {
      this.$refs.addFormRef.validate((valid) => {
        if (!valid) return;
        this.sort = postD(product_typeReleaseApi(), this.addForm).then(
          (res) => {
            if (res.code == "200") {
